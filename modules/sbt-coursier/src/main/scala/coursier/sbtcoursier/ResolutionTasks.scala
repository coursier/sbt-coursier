...
import coursier.{Cache, Fetch, FromSbt, ProjectCache, Resolution}
  ...
  private def resolution(
    ...
    val res = try {
      ...
      val fetch = Fetch.from(
        params.repositories,
        Cache.fetch(params.cache, params.cachePolicies.head, checksums = params.checksums, logger = Some(resLogger), pool = pool, ttl = params.ttl),
        params.cachePolicies.tail.map(p =>
          Cache.fetch(params.cache, p, checksums = params.checksums, logger = Some(resLogger), pool = pool, ttl = params.ttl)
        ): _*
      )
